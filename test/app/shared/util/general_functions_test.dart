...
void main() {
  ...
  test(
    'Deve retornar data formato BR tipo String (toBrazilTime) ...',
    () async {
      DateTime? date = DateTime.now();
      ...
      when(
        () => funcions.toBrazilTime("2021-11-02"),
  ...
  test(
    'Deve retornar resultado de conveção em tipo String (realToCoin) ...',
    () async {
  ...
  test(
    ...
    () async {
      ...
      when(
        () => funcions.calcRealToCoin('1.00', '1.00'),
  ...
  test(
    'Deve retornar String vazia em caso de erro (calcRealToCoin) ...',
    () async {
      var priceCoin = "";
  ...
  test(
    ...
    () async {
      ...
      when(
        () => funcions.calcCoinToReal('1.00', '1.00'),
      ).thenAnswer((_) => priceCoin);
      //act
      //assert
      expect(priceCoin, isA<String?>());
  ...
  test(
    'Deve retornar String vazia em caso de erro (calcCoinToReal) ...',
    () async {
      var priceCoin = "";
  ...
  test(
    ...
    () async {
      ...
      when(
        () => funcions.formatNumberBitCoin('361.000000'),
  ...
  test(
    ...
    () async {
      ...
      when(
        () => funcions.formatNumberBr('361.010000'),
      ).thenAnswer((_) => resnumber);
      //act
      //assert
  ...
  test(
    'Deve retornar 0 como String em caso de erro (formatNumberBr) ...',
    () async {
      var resnumber = "0";

      when(
        () => funcions.formatNumberBr('361.000000'),
      ).thenAnswer((_) => resnumber);
      //act
      //assert
      expect(resnumber, isA<String?>());
  ...
  test(
    ...
    () async {
      ...
      when(
        () => funcions.formatNumberUs('361.010100'),
      ).thenAnswer((_) => resnumber);
      //act
      //assert
      var numBr = formatter.format(double.parse(resnumber));
  ...
  test(
    'Deve retornar 0 como String em caso de erro (formatNumberUs) ...',
    () async {
      var resnumber = "0";

      when(
        () => funcions.formatNumberUs('361.010000'),
      ).thenAnswer((_) => resnumber);
      //act
      //assert
      expect(resnumber, isA<String?>());
